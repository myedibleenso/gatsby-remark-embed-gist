...
describe("gatsby-remark-embedded-gist", () => {
  describe("core use cases", () => {
    ...
    it("generates an embedded gist with default username", async () => {
      const markdownAST = remark.parse(
        "`gist:ce54fdb1e5621b5966e146026995b974`"
      );
  ...
  describe("option: truncate can reduce html bulk", () => {
    it("query option: 'query.truncate' generates stripped down gist HTML", async () => {
      const markdownAST = remark.parse(
        "`gist:weirdpattern/ce54fdb1e5621b5966e146026995b974?file=example.sh&truncate=true`"
      );
      const processed = await plugin({ markdownAST });
      expect(processed).toBeTruthy();
      ...
      // console.log(html);
      expect(html).toMatch(/\bb-c\b/gim);
    });
    it("global option: 'options.truncate' generates stripped down gist HTML", async () => {
      const markdownAST = remark.parse(
        "`gist:weirdpattern/ce54fdb1e5621b5966e146026995b974?file=example.sh`"
      );
